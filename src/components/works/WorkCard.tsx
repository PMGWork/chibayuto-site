  ...
  return (
    <div className="flex flex-col gap-4 pb-8">
      <a href={`/works/${work.id}`}>
        <div
          className="relative w-full overflow-hidden rounded-xl corner-lg"
        ...
        >
          {optimizedImage ? (
            ...
            <div className="absolute top-0 left-0 flex h-full w-full items-center justify-center rounded-xl corner-lg bg-gray-200">
      ...
      <div className="flex flex-col gap-2">
        ...
        <div className="flex gap-2">
          {workTags.map((tag, tagIndex) => (
            <span
              key={tagIndex}
              className="rounded-lg corner-md border border-gray-200 bg-gray-50 px-2 py-1 text-xs font-medium"
            >
              {tag}
            </span>
          ))}
